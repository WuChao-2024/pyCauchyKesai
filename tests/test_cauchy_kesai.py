...
class TestIONArray:
    ...
    def test_basic_construct(self):
        ...
        assert arr.as_array().shape == (1, 3, 224, 224)
        assert arr.as_array().dtype == np.float32
        assert arr.phy_addr > 0
        assert arr.mem_size > 0

    def test_phy_addr_accessible(self):
        """物理地址可获取"""
    ...
    def test_read_write(self):
        ...
        arr.as_array()[:] = np.arange(10, dtype=np.float32)
        assert arr.as_array()[0] == 0
        assert arr.as_array()[9] == 9
